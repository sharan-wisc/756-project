...
static Options options_filter(Options Z, Nat len){
  ...
  Options o;
	Options wo_inferior;
	Nat iterator;
	Time rat = -1.797693e+308;
	Capacitance load = 1.797693e+308;
	for (o = Z, iterator = 0; o, iterator < len; o = ONEXT(o), iterator++) {
/*		if(OTIME(wo_inferior) < OTIME(o)){
			OTIME(wo_inferior) = OTIME(o);
			OLOAD(wo_inferior) = OLOAD(o);			
		}
		else if(OTIME(wo_inferior) = OTIME(o)){
			if(OLOAD(wo_inferior) > OLOAD(o)){
				OTIME(wo_inferior) = OTIME(o);
				OLOAD(wo_inferior) = OLOAD(o);			
			}
		}*/
...
//	printf("rat = %lf\n", rat);
//	printf("load = %lf\n", load);
  wo_inferior = option_mk(pair_mk(rat, load));
//	Z = wo_inferior;
	return wo_inferior;
